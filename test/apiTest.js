...
async function test() {
  console.log('测试 API /api/preview ...\n');
  ...
  try {
    ...
    console.log('  pngBuffer 长度:', result.pngBuffer ? result.pngBuffer.length : 0);

    if (result.dataUrl && result.dataUrl.startsWith('data:image/png;base64,')) {
      console.log('✓ Base64 格式正确');
    } else {
      console.log('✗ Base64 格式错误');
    }

    console.log('\n=== API 测试完成 ===');
    console.log('预览图片已生成，可以在 http://localhost:3000 查看');
