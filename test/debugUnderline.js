...
console.log('前 30 个节点的下划线状态:');
let lastUnderline = false;
nodes.slice(0, 30).forEach((node, i) => {
  if (node.type === 'underline') {
    lastUnderline = node.value;
    console.log(`[${i}] underline: ${node.value}`);
  } else if (node.type === 'text') {
    console.log(`[${i}] text: "${node.value.substring(0, 20)}..." (underline=${lastUnderline})`);
